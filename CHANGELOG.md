...
- **并行工作台**：职位采集、AI 评分与招呼语生成可以按资源并行执行，也可以只启动其中一步；新增任务完成弹窗与运行记录，投递与监测仍共用浏览器安全通道并保持互斥。
- **采集加速**：新增搜索预筛选与并行采集编排，支持在独立标签页中安全并行处理多个平台；更高风险的全平台并行模式作为试点默认关闭，需在配置页明确开启。
- **并行 AI 评分默认开启**：默认使用 2 个并发工作器，可在配置页调整为 1–3；模型请求并行，岗位状态与评分写入保持串行可靠。
- **51job 真实详情 URL**：适配前程无忧搜索列表页改版，采集结果可打开真实岗位详情页，并补充对应回归测试。
- **智联登录弹窗容错**：详情页正常内容旁的登录 CTA 不再被误判为登录墙；只有连续 3 个被登录墙遮挡的详情页才会暂停智联队列，并提示用户重新登录后重试。
- **工程与稳定性**：加入 `uv.lock` 保证依赖可复现，并补充采集编排、AI Token、配置、Web API 等回归测试。

### 验证

- 智联采集器回归测试通过（18 passed）。
